        ...
        else
        {
            switch (buttonString)
            {
                case "delete":
                    ...
                    break;
                case "clear":
                    // Instead of directly clearing text with Remove,
                    // we clear the input field and trigger its OnEndEdit event.
                    inputTarget.onSubmit.Invoke(inputTarget.text);
                    inputTarget.text = "";
                    GetInputFieldTarget.Index = 0;
#if(UNITY_EDITOR)
                    print("Input field cleared and OnEndEdit event invoked.");
#endif
                    break;
                case "backward":
                    if(GetInputFieldTarget.Index > 0)
